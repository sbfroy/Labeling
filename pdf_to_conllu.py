...
def process_text_with_spacy(text, nlp):
    ...
    for sent in doc.sents:
        tokens = []
        for token in sent:
            space_after = 'SpaceAfter=No' if token.whitespace_ == '' else ''

            # Further split tokens (punctuations) to separate symbols
            #split_tokens = re.findall(r"\w+|[^\w\s]", token.text, re.UNICODE)
            split_tokens = re.findall(r"\w+(?:[-/.]\w+)*|[^\w\s]", token.text, re.UNICODE)
            for sub_token in split_tokens:
                tokens.append((sub_token, space_after))

        sentences.append(tokens) # Each sent is a list of tuples 
...
def to_conllu(sentences):
    ...
    for sent_id, tokens in enumerate(sentences, start=1):
        ...
        meta_text = []
        for i, (token, space_after) in enumerate(tokens):
            meta_text.append(token)
            if space_after != "SpaceAfter=No" and i < len(tokens) - 1:  
                meta_text.append(" ")
        
        conllu_lines.append(f'# text = {"".join(meta_text)}') # metadata
    
        for token_id, (token, space_after) in enumerate(tokens, start=1):
            lemma = upos = xpos = feats = head = deprel = deps = '_'
            misc = f'{space_after}|name=O' if space_after else 'name=O'
            conllu_lines.append(
